...
public class ExcelController {
	...
	@GetMapping("excel")
	public HashMap<String, Object> handleRequestInternal(HttpServletRequest request, HttpServletResponse response) {
		...
		try {
			...
			String parameter = request.getParameter("sendData").replace("\\", "").replace("[", "").replace("]", "")
					.replace("}\"", "}").replace("\"{", "{");
			...
			String localFile = "C:\\dev\\nginx\\nginx-1.20.2\\html\\excel\\" + "인사고과" + ".xlsx";
